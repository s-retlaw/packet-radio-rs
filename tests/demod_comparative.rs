...
fn test_fast_demod_on_all_scenarios() {
    ...
    for scenario in standard_test_scenarios() {
        ...
        let mut symbols = vec![DemodSymbol { bit: false, llr: 0, sample_idx: 0, raw_bit: false }; 2000];
...
fn test_adaptive_tracker_locks_on_preamble() {
    ...
    let mut demod = QualityDemodulator::new(config);
    let mut symbols = vec![DemodSymbol { bit: false, llr: 0, sample_idx: 0, raw_bit: false }; 1000];
...
fn test_quality_path_outperforms_fast_on_noise() {
    ...
    let mut fast_syms = vec![DemodSymbol { bit: false, llr: 0, sample_idx: 0, raw_bit: false }; 100000];
    let fast_count = fast_demod.process_samples(&noisy, &mut fast_syms);

    // Run quality path
    let mut qual_demod = QualityDemodulator::new(config);
    let mut qual_syms = vec![DemodSymbol { bit: false, llr: 0, sample_idx: 0, raw_bit: false }; 100000];
