...
pub fn run_smart3(path: &str) {
    ...
    println!(
        "  Smart3:  {:>4} packets in {:.2}s ({:.0}x real-time, {} soft saves)",
        smart3.frames.len(),
    ...
    println!(
        "  Fast:    {:>4} packets (baseline comparison)",
        fast.frames.len()
    );
    let gain = smart3.frames.len() as i64 - fast.frames.len() as i64;
    println!(
        "  Gain:    {:>+4} packets ({:.1}% improvement over fast)",
        gain,
        if !fast.frames.is_empty() {
            gain as f64 / fast.frames.len() as f64 * 100.0
        } else {
            0.0
        }
    );
...
pub fn run_smart3_sweep(path: &str) {
    ...
    let bpf_types: [(
        &str,
        Box<dyn Fn(u32, f64) -> packet_radio_core::modem::filter::BiquadFilter>,
    ); 3] = [
        (
            "narrow",
            Box::new(|sr, center| filter::bandpass_coeffs(sr, center, 1200.0)),
        ),
        (
            "std",
            Box::new(|sr, center| filter::bandpass_coeffs(sr, center, 1600.0)),
        ),
        (
            "wide",
            Box::new(|sr, center| filter::bandpass_coeffs(sr, center, 2000.0)),
        ),
    ];
    ...
    for (i, r) in results.iter().take(15).enumerate() {
        println!(
            "    {:>2}. freq={:>+4} bpf={:>6} t{}: {} pkts ({} exclusive)",
            i + 1,
            r.freq_off,
            r.bpf_type,
            r.timing,
    ...
    let narrow_results: Vec<&SweepResult> = results
        .iter()
        ...
        for (d1_bpf_name, d1_bpf_fn) in &d1_bpf_types {
            for &d1_t in &timing_phases {
                for &(d2_t, d3_t) in &d23_pairs {
                    ...
                    let mut d1 = FastDemodulator::new(config)
                        .filter(bpf1)
                        .phase_offset(off1)
                        .frequencies(mark1, space1)
                        .with_energy_llr();
                    let mut d2 = FastDemodulator::new(config)
                        .filter(narrow)
                        .phase_offset(off2)
                        .with_energy_llr();
                    let mut d3 = FastDemodulator::new(config)
                        .filter(narrow)
                        .phase_offset(off3)
                        .with_energy_llr();
    ...
    println!(
        "  {:>6} {:>5} {:>8} {:>8}  {:>5}",
        "D1freq", "D1bpf", "D1/D2/D3", "timings", "Total"
    );
    println!("  {}", "─".repeat(45));
    ...
    println!(
        "\n  Current Smart3: freq-50/wide/t2 + narrow/t0 + narrow/t1 = {}",
        smart3_set.len()
    );
    println!();
}
